...
//            inputGrid.add(new Label(String.format("%.2f", (double) items.get(i).getPrice() / 100)), 4, j);
            ...
            ReWrite name = new ReWrite(item.getName());
            name.setOnFocusLost(eh -> {
                item.setName(name.getText());
            });
            inputGrid.add(name, 0, j);
            ReWrite type = new ReWrite(item.getType());
            type.setOnFocusLost(eh -> {
                item.setType(type.getText());
            });
            inputGrid.add(type, 1, j);
            
            String combinedSt = String.format("%.2f", item.getAmount() * item.getPrice() / 100.0);
            String amountSt = String.format("%s", item.getAmount());
            String induvidualSt = String.format("%.2f", item.getPrice() / 100.0);
            ReWrite combined = new ReWrite(combinedSt);
            inputGrid.add(combined, 2, j);
            ReWrite amount = new ReWrite(amountSt);
            inputGrid.add(amount, 3, j);
            ReWrite induvidual = new ReWrite(induvidualSt);
            inputGrid.add(induvidual, 4, j);
...
//                    warnig.setText("Combindevalue incorrect");
    ...
    public static void open(BudgetManager manager, Stage mainStage) {
        ...
        AutoCompleteTextField type = new AutoCompleteTextField();
        try {
//            Set<String> types = manager.getTypes();
            SortedSet<String> types = new TreeSet<>();
            manager.getTypes().stream().forEach(e -> {
                types.add(e); 
            });
            type.setEntries(types);
